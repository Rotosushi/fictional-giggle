/* Generated by re2c 1.2.1 on Tue Jul  7 09:17:53 2020 */
#line 1 "lexer.re"
...
/*
    ...
    as in, to build it in the other way would just mean refactoring
    this code to this design later, so why not just do it now.
    (the designs are fairly different.)
    ...
    be less to change to fit with the extensions (each situation
    is a case of the full input doesn't fit in the memory buffer
    we are parsing.).)
    ...
    the language. and silently we would lex this as
    (user input) ';' empty
  ...
  - get user input:
    ...
    - enter something valid, keep trying to
        ...
        once we find the end of the term
          (according to the rules of the grammar.)
        - then we type the term
          - either print the type or the type error
        - if we have some type, then we evaluate the term
          - either print the result, or the evaluation error.
        then loop to getting user input.

    to recap:
    assuming the existance of the term [term ';' term] within the grammar,
    ...
    'period' character (homomorphically an english period character.
    (aside: homomorphism is what i would call a metaphor
    if it were between two entities in reality being described by
     spoken/written languages.))
    and the input consisting of "" will cause the parser to end immediately.
...
/*
    ...
    expects the user to store them like this. hence,
    we have a reentrant lexer, without specifying the
    re2c lexer as reentrant.
*/
